...
{
    protected static $defaultName = 'jacquesndl:easy-admin-plus:user:enable';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->dispatcher->dispatch(new GenericEvent($user), EasyAdminPlusEvents::USER_PRE_ENABLE);

        $user->setEnabled(true);
        $this->em->flush();

        $this->dispatcher->dispatch(new GenericEvent($user), EasyAdminPlusEvents::USER_POST_ENABLE);

        $io->success(sprintf('User "%s" enabled', $username));
